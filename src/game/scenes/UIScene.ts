...
export class UIScene extends Phaser.Scene {
  ...
  private playerOnePanel!: Phaser.GameObjects.Rectangle;
  private playerTwoPanel!: Phaser.GameObjects.Rectangle;
  private playerOneTitle!: Phaser.GameObjects.Text;
  ...
  private onHudUpdate(hud: HudState): void {
    ...
    this.detailText.setText(
      `Angle ${hud.angle.toFixed(1)} deg  |  Power ${hud.power.toFixed(0)}  |  Wind ${windDirection} ${Math.abs(hud.wind).toFixed(1)}`
    );
  ...
  private updateTurnHighlight(currentPlayer: 0 | 1): void {
  ...
  private redrawWindMeter(wind: number): void {
