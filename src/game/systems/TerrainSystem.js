...
import { EXPLOSION_RADIUS, GAME_WIDTH, MAX_BUILDING_HEIGHT, MIN_BUILDING_HEIGHT, STREET_Y } from '../constants';
...
export class TerrainSystem {
    ...
    applyExplosion(x, y, radius = EXPLOSION_RADIUS) {
        for (const building of this.buildings) {
            if (x + radius < building.x || x - radius > building.x + building.width || y - radius > building.baseY) {
                continue;
            }
            ...
            for (let i = 0; i < cols.length; i += 1) {
                ...
                const nextTop = Phaser.Math.Clamp(Math.max(currentTop, carveBottomY), building.topY, building.baseY - MIN_COLUMN_HEIGHT);
    ...
    updateBuildingVisuals(building, healthRatio) {
    ...
    redrawBuildingShell(building, healthRatio) {
        const shell = building.shell;
        if (!shell) {
            return;
        }
        shell.clear();
        const tint = Phaser.Display.Color.Interpolate.ColorWithColor(Phaser.Display.Color.ValueToColor(0x1b2437), Phaser.Display.Color.ValueToColor(building.baseColor), 100, Math.floor(healthRatio * 100));
    ...
    redrawBuildingDetails(building, healthRatio) {
